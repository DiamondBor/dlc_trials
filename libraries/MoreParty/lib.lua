...
function Lib:init()
    ...
    Utils.hook(DarkEquipMenu, "drawAbilityPreview", function(orig, self, index, x, y, abilities, compare)
        if not Mod.trapped then
            ...
            love.graphics.print("(Locked.)", x + 26, y - 6)
        end
    end)
    ...
    Utils.hook(DarkPowerMenu, "drawStats", function(orig, self)
        if not Mod.trapped then
            ...
            for i = 1, 3 do
                ...
                love.graphics.print("(Trapped.)", x, y)
                love.graphics.pop()
            end
        end
    end)
    ...
    Utils.hook(DarkMenuPartySelect, "draw", function(orig, self)
        if #Game.party <= 4 and not Kristal.getLibConfig("moreparty", "scroller") or #Game.party <= 3 then
            if not Mod.trapped then
                orig(self)
            else
                for i,party in ipairs(Game.party) do
                    ...
                    local function spawn(of_x, of_y, v)
                        if self.selected_party ~= i then
                            Draw.setColor(v, v, v, 0.4)
                        else
                            Draw.setColor(v, v, v, 1)
                        end
                        Draw.draw(Assets.getTexture(party:getMenuIcon()), ((i-1)*50 + (ox*2))+of_x, (oy*2)+of_y, 0, 2, 2)
                    end
                    spawn(-2, -2, 1)
                    spawn(-2, 2, 1)
                    spawn(2, -2, 1)
        ...
        if not Kristal.getLibConfig("moreparty", "scroller") then
            ...
            for i,party in ipairs(Game.party) do
                ...
                local function spawn(of_x, of_y, v)
                    if self.selected_party ~= i then
                        local nv = (v == 1 and 0.5) or 0
                        Draw.setColor(nv, nv, nv, 1)
                    else
                        Draw.setColor(v, v, v, 1)
                    end
                    Draw.draw(Assets.getTexture(party:getMenuIcon()), ((i-1)*50 + (ox*2) - (self.shown - (Kristal.getLibConfig("moreparty", "classic_mode") and 3 or 4)) * 50)+of_x, (oy*2)+of_y, 0, 2, 2)
                end
                spawn(-2, -2, 1)
                spawn(-2, 2, 1)
                spawn(2, -2, 1)
    ...
    Utils.hook(ActionBox, "init", function(orig, self, x, y, index, battler)
        ...
        if self.head_sprite then
            self.head_sprite.x = -500
            self.head_sprite.y = -500
        end
        if self.name_sprite then
            self.name_sprite.x = self.name_sprite.x - 28
        end
    end)
    ...
    local function OverworldActionBox_Draw(self)
        -- Draw the line at the top
        if self.selected then
            love.graphics.setColor((self.chara.id == "dess" and self.chara:getColor()) or {1,1,1})
        else
        ...
        -- Draw health
        love.graphics.setColor((self.chara.id == "dess" and PALETTE["action_health_bg"]) or {1,1,1})
    ...
    Utils.hook(OverworldActionBox, "draw", function(orig, self)
        Object.draw(self)
        if #Game.party <= 3 then OverworldActionBox_Draw(self) return end
        ...
        if not Kristal.getLibConfig("moreparty", "classic_mode") then
            ...
            -- Draw health
            Draw.setColor((self.chara.id == "dess" and PALETTE["action_health_bg"]) or {1,1,1})
            love.graphics.rectangle("fill", 128, y_bar, 76, 9)

            local health = (self.chara:getHealth() / self.chara:getStat("health")) * 76

            if health > 0 then
                Draw.setColor((self.chara.id == "dess" and self.chara:getColor()) or {1,1,1})
